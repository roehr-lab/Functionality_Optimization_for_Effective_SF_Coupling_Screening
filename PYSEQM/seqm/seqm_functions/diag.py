...
def sym_eig_trunc(x,nheavyatom,nH,nocc, eig_only=False):
    ...
    device = x.device
    if x.dim()==2:
        e0, v = torch.linalg.eigh(pack(x, nheavyatom, nH), UPLO='U') #e0,v = torch.symeig(pack(x, nheavyatom, nH),eigenvectors=True,upper=True)
        ...
        try:
            e0, v = torch.linalg.eigh(x0, UPLO='U') #e0,v = torch.symeig(x0,eigenvectors=True,upper=True)
        except:
